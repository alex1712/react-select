   ...
   */
	...
	handleInputChange: function handleInputChange(event) {
		...
		if (this.props.asyncOptions) {
			...
			filteredOptions = this.addNewTagPlaceholder(filteredOptions, event.target.value);
	...
	loadAsyncOptions: function loadAsyncOptions(input, state) {
		...
		for (var i = 0; i <= input.length; i++) {
			...
			if (this._optionsCache[cacheKey] && (input === cacheKey || this._optionsCache[cacheKey].complete)) {
				...
				filteredOptions = this.addNewTagPlaceholder(filteredOptions, input);
	...
	addNewTagPlaceholder: function addCreateOption(options, input) {
